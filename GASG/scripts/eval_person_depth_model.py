...
from scripts.eval_literature_rightview_generation import (
    DAV2DisparityEstimator,
    Mono2StereoDIBRGenerator,
    PseudoStereo3DODGenerator,
    load_deep3d_generator,
)
...
METHOD_INFO: Dict[str, Dict[str, str]] = {
    ...
    "defom_deep3d_svsm": {
        "pretty": "Deep3D/SVSM right view -> DEFOM-Stereo",
        "type": "PersonDataset-trained right-view generator",
        "scale": "metric",
    },
    "defom_pseudostereo_3dod": {
        "pretty": "PseudoStereo-3DOD right view -> DEFOM-Stereo",
        "type": "published pseudo-stereo reproduction",
        "scale": "metric",
    },
    "defom_mono2stereo_dibr": {
        "pretty": "Mono2Stereo-DIBR right view -> DEFOM-Stereo",
        "type": "warp-and-inpaint stereo conversion",
        "scale": "metric",
    },
    "defom_moge_warp": {
    ...
    "defom_zerostereo_tuned": {
        "pretty": "ZeroStereo/StereoGen tuned right view -> DEFOM-Stereo",
        "type": "PersonDataset-tuned pseudo-stereo generator",
        "scale": "metric",
    },
    "defom_stereospace": {
...
class DefomPredictor(Predictor):
    def __init__(self, args: argparse.Namespace, device: torch.device,
        ...
        elif mode == "deep3d_svsm":
            self.deep3d = load_deep3d_generator(args.deep3d_ckpt, device)
        elif mode == "zerostereo":
            self.zerostereo = load_zerostereo_map(args.zerostereo_work_root)
        elif mode == "zerostereo_tuned":
            self.saved_right = load_saved_right_map(
                args.zerostereo_tuned_work_root, "ZeroStereo/StereoGen tuned")
        ...
        elif mode == "pseudostereo_3dod":
            self.right_generator = PseudoStereo3DODGenerator(
                DAV2DisparityEstimator(device, target_max_disp=args.dav2_target_disp)
            )
        elif mode == "mono2stereo_dibr":
            self.right_generator = Mono2StereoDIBRGenerator(
                DAV2DisparityEstimator(device, target_max_disp=args.dav2_target_disp)
            )
        elif mode == "moge_warp":
    ...
    def _right(self, sample: Dict, index: int, left: torch.Tensor) -> torch.Tensor:
        ...
        if self.mode == "deep3d_svsm":
            assert self.deep3d is not None
            return self.deep3d(sample["left"].unsqueeze(0).to(self.device))
        if self.mode == "zerostereo":
            ...
            return torch.from_numpy(arr).permute(2, 0, 1).unsqueeze(0).to(
                self.device)
        if self.mode == "zerostereo_tuned":
            assert self.saved_right is not None
            path = self.saved_right.get(int(index))
            if path is None or not os.path.exists(path):
                raise FileNotFoundError(
                    f"No tuned ZeroStereo right view for PersonDataset index {index}")
            img = Image.open(path).convert("RGB").resize(
                (self.args.width, self.args.height), Image.BILINEAR)
            arr = np.asarray(img, dtype=np.float32) / 255.0
            return torch.from_numpy(arr).permute(2, 0, 1).unsqueeze(0).to(
...
def build_predictor(method: str, args: argparse.Namespace,
    ...
    if method == "defom_deep3d_svsm":
        return DefomPredictor(args, device, "deep3d_svsm")
    if method == "defom_pseudostereo_3dod":
        return DefomPredictor(args, device, "pseudostereo_3dod")
    if method == "defom_mono2stereo_dibr":
        return DefomPredictor(args, device, "mono2stereo_dibr")
    ...
    if method == "defom_zerostereo_tuned":
        return DefomPredictor(args, device, "zerostereo_tuned")
...
def evaluate(args: argparse.Namespace) -> Dict:
    ...
    if args.use_saved_indices and args.method == "defom_stereospace":
        ...
        if args.max_samples is not None:
            indices = indices[:args.max_samples]
    elif args.use_saved_indices and args.method == "defom_zerostereo_tuned":
        indices = sorted(load_saved_right_map(
            args.zerostereo_tuned_work_root, "ZeroStereo/StereoGen tuned"))
        if args.max_samples is not None:
            indices = indices[:args.max_samples]
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--zerostereo_tuned_work_root",
                   default="data/zerostereo_person_tuned_test")
