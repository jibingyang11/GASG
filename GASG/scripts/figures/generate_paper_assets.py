...
def create_visual_figures() -> None:
    ...
    methods = [
        ...
        ("ZeroStereo FT", "defom_zerostereo_tuned", "pred"),
...
def create_tables(right_rows: List[Dict], drows: List[Dict]) -> None:
    ...
    write_table(
        ...
        "Right-view generation quality on PersonDataset under the fair target-domain protocol. ZeroStereo/StereoGen is fine-tuned on the PersonDataset training split; DA-V2 and MoGe use PersonDataset-calibrated warp scales; GASG uses the trained task-specialized checkpoint.",
