...
def run_official_generation(args) -> float:
    ...
    if args.stereogen_model_path:
        stereo_cmd.append(
            "model.instance.pretrained_model_name_or_path="
            + as_posix(os.path.abspath(args.stereogen_model_path))
        )
...
def evaluate_generated(args, metadata: Dict,
    ...
    row = {
        ...
        "device": "cuda",
        "params_M": count_stereogen_params(
            args.zerostereo_repo, args.stereogen_model_path),
    ...
    out_path = os.path.join(args.save_dir, f"{args.method_name}.json")
...
def count_stereogen_params(zerostereo_repo: str,
                           stereogen_model_path: Optional[str] = None) -> float:
    root = stereogen_model_path or os.path.join(
...
def main() -> None:
    ...
    parser.add_argument("--method_name", default="zerostereo_official")
    parser.add_argument("--stereogen_model_path", default=None)
