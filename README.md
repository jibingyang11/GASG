...
GASG is a lightweight left-to-right image generation model trained for the pseudo-stereo depth estimation task. The core process is：

    single left image -> GASG -> pseudo right image
    left image + pseudo right image -> stereo depth estimator -> absolute depth

The contribution of the paper is **the GASG pseudo-right image generator and its pluggable binocular interface**, not a fixed `GASG + DEFOM-Stereo` combination. In this project, DEFOM-Stereo is only a representative stereo backend used for experimental validation.
